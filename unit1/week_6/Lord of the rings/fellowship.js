...
const makeMiddleEarth = () => {
  ...
  for (types of lands) {
    let $article = $('<article>').attr('id' , types);
    $article.appendTo($section);
    let $h1 = $('<h1>').text(types).appendTo($article);
  }
...
const makeHobbits = () => {
  ...
  for (names of hobbits) {
    let $li = $('<li>').text(names).addClass('hobbit').appendTo($ul);
    $('#The-Shire').append($ul);
  }
...
const makeBuddies = () => {
  ...
  const $aside = $('<aside>').appendTo('#middle-earth');
  // 2. display an unordered list of buddies in the aside
  const $ulBuddies = $('<ul>')
  for (chooseBuddies of buddies){
    let $li = $('<li>').text(chooseBuddies).addClass('buddy');
    $li.appendTo($ulBuddies).appendTo($aside);
  }
...
const theBalrog = () => {
  ...
  $('.buddy').eq(0).removeClass().addClass('the-white');
